...
export function getBoxContentBounds(box) {
    return {
        left: box.left + 1,
        top: box.top + 1,
        right: box.right - 1,
...
export function getBoundsUnion(objects) {
    if (objects.length === 0)
        return null;
...
export function getObjectSelectionBounds(object) {
...
export function getBoxCornerPoints(box) {
    return {
...
export function getLineEndpointPoints(line) {
    return {
...
export function getObjectRenderCells(object) {
    switch (object.type) {
        case "box": {
            const cells = new Map();
            const add = (x, y) => {
        ...
        case "line":
            return getLineRenderCells({ x: object.x1, y: object.y1 }, { x: object.x2, y: object.y2 }, object.style);
        case "elbow":
            return getElbowRenderCells({ x: object.x1, y: object.y1 }, { x: object.x2, y: object.y2 }, object.style, object.orientation);
        case "paint":
...
export function objectContainsPoint(object, x, y) {
    switch (object.type) {
        case "box": {
        ...
        case "line":
            return getLineRenderCells({ x: object.x1, y: object.y1 }, { x: object.x2, y: object.y2 }, object.style).some((point) => point.x === x && point.y === y);
        case "elbow":
            return getElbowRenderCells({ x: object.x1, y: object.y1 }, { x: object.x2, y: object.y2 }, object.style, object.orientation).some((point) => point.x === x && point.y === y);
        case "paint":
