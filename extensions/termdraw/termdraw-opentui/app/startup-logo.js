...
export function renderStartupLogo(frameBuffer, state, chromeMode, layout, startupLogoEnabled, startupLogoDismissed) {
    if (!startupLogoEnabled || startupLogoDismissed)
        return;
    ...
    const availableWidth = chromeMode === "full" && layout ? layout.dividerX - state.canvasLeftCol : state.width;
    const availableHeight = chromeMode === "full" && layout ? layout.bodyBottom - state.canvasTopRow + 1 : state.height;
    ...
    if (showCaption) {
        ...
        frameBuffer.drawText(STARTUP_LOGO_CAPTION, captionX, captionY, getStartupLogoCaptionColor(), COLORS.panel, TextAttributes.DIM);
    }
}
