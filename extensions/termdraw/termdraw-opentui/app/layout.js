...
import { BOX_STYLE_OPTIONS, BRUSH_OPTIONS, COLOR_SWATCH_COLUMNS, COLOR_SWATCH_WIDTH, COLORS, ELBOW_STYLE_OPTIONS, LINE_STYLE_OPTIONS, STYLE_BUTTON_WIDTH, TEXT_BORDER_OPTIONS, TOOL_BUTTON_HEIGHT, TOOL_BUTTON_WIDTH, TOOL_PALETTE_WIDTH, } from "./theme.js";
...
export function isInsideRect(x, y, left, top, width, height) {
    return x >= left && x < left + width && y >= top && y < top + height;
}
/** Returns the number of style rows rendered beneath the active tool. */
export function getContextualStyleRowCount(currentMode) {
    if (currentMode === "box")
        return BOX_STYLE_OPTIONS.length;
    if (currentMode === "line")
        return LINE_STYLE_OPTIONS.length;
    if (currentMode === "elbow")
        return ELBOW_STYLE_OPTIONS.length;
    if (currentMode === "paint")
        return BRUSH_OPTIONS.length;
    if (currentMode === "text")
        return TEXT_BORDER_OPTIONS.length;
    return 0;
}
/** Returns the palette x-position used by tool buttons and swatches. */
function getPaletteButtonLeft(layout) {
    return layout.paletteLeft + 1;
}
/** Returns the clickable tool buttons for the current palette layout. */
export function getToolButtons(layout, currentMode) {
...
export function getContextualStyleButtons(layout, currentMode) {
    if (currentMode !== "box" &&
        currentMode !== "line" &&
        currentMode !== "elbow" &&
        currentMode !== "paint" &&
    ...
    const buttonLeft = getPaletteButtonLeft(layout);
    const activeButton = getToolButtons(layout, currentMode).find((button) => button.mode === currentMode);
    if (!activeButton)
        return [];
    const options = currentMode === "box"
        ? BOX_STYLE_OPTIONS
        : currentMode === "line"
            ? LINE_STYLE_OPTIONS
            : currentMode === "elbow"
...
export function getColorSwatches(layout, colors) {
    const buttonLeft = getPaletteButtonLeft(layout);
...
export function getDiagramSavePromptLayout(width, height, prompt) {
    if (!prompt)
        return null;
    ...
    const contentWidth = Math.max(24, Math.min(width - 6, Math.max(visibleCellCount(label), visibleCellCount(displayPath), visibleCellCount(helperLine)) + 2));
...
export function isCanvasChromeEvent(canvasLeftCol, canvasTopRow, layout, x, y) {
    return (x >= canvasLeftCol && x <= layout.dividerX - 1 && y >= canvasTopRow && y <= layout.bodyBottom);
}
