...
import { BOX_STYLE_OPTIONS, BRUSH_OPTIONS, COLORS, ELBOW_STYLE_OPTIONS, LINE_STYLE_OPTIONS, MIN_HEIGHT, MIN_WIDTH, TEXT_BORDER_OPTIONS, getInkColorContrast, getInkColorValue, } from "./theme.js";
...
export function drawTooSmallMessage(frameBuffer, width, height) {
    const lines = [
    ...
    for (let index = 0; index < lines.length; index += 1) {
        ...
        frameBuffer.drawText(text, x, startY + index, COLORS.warning, COLORS.panel, TextAttributes.BOLD);
    }
}
/** Draws the full-chrome frame, header, divider, and footer rows. */
export function drawChrome(frameBuffer, width, height, state, layout, footerTextOverride, canSaveDiagram) {
    ...
    drawFooterRow(frameBuffer, width, state.currentStatus, layout, footerTextOverride, canSaveDiagram);
}
/** Draws the header row describing the active tool, style, and color. */
function drawHeaderRow(frameBuffer, width, state, layout) {
    const y = 1;
    ...
    const modeColor = state.currentMode === "select"
        ? COLORS.select
    ...
    if (state.currentMode === "paint") {
        ...
        x = drawSegment(frameBuffer, x, y, brush ? `${brush.sample} ${brush.label}` : `"${state.currentBrush}"`, COLORS.paint, COLORS.panel);
    }
    else if (state.currentMode === "box") {
        const boxStyle = BOX_STYLE_OPTIONS.find((option) => option.style === state.currentBoxStyle) ??
            BOX_STYLE_OPTIONS[0];
        x = drawSegment(frameBuffer, x, y, "  style:", COLORS.dim, COLORS.panel);
        x = drawSegment(frameBuffer, x, y, `${boxStyle.sample} ${boxStyle.label}`, COLORS.warning, COLORS.panel);
    }
    else if (state.currentMode === "line" || state.currentMode === "elbow") {
        const lineStyleOptions = state.currentMode === "elbow" ? ELBOW_STYLE_OPTIONS : LINE_STYLE_OPTIONS;
        const lineStyle = lineStyleOptions.find((option) => option.style === state.currentLineStyle) ??
            lineStyleOptions[0];
        x = drawSegment(frameBuffer, x, y, "  style:", COLORS.dim, COLORS.panel);
        x = drawSegment(frameBuffer, x, y, `${lineStyle.sample} ${lineStyle.label}`, COLORS.accent, COLORS.panel);
    }
    else if (state.currentMode === "text") {
        const textBorder = TEXT_BORDER_OPTIONS.find((option) => option.style === state.currentTextBorderMode) ??
            TEXT_BORDER_OPTIONS[0];
        x = drawSegment(frameBuffer, x, y, "  border:", COLORS.dim, COLORS.panel);
        x = drawSegment(frameBuffer, x, y, `${textBorder.sample} ${textBorder.label}`, COLORS.success, COLORS.panel);
    }
    x = drawSegment(frameBuffer, x, y, "  color:", COLORS.dim, COLORS.panel);
    drawSegment(frameBuffer, x, y, "●", getInkColorValue(state.currentInkColor), COLORS.panel, TextAttributes.BOLD);
    const paletteTitle = padToWidth("Tools", paletteWidth);
    frameBuffer.drawText(paletteTitle, layout.dividerX + 1, y, COLORS.dim, COLORS.panel, TextAttributes.BOLD);
}
/** Draws the divider row that separates the header from the main content. */
function drawHeaderDivider(frameBuffer, width, layout) {
    const y = 2;
...
function drawFooterRow(frameBuffer, width, status, layout, footerTextOverride, canSaveDiagram) {
    const text = footerTextOverride ??
        `B Brush • A Select • U Box • P Line • E Elbow • T Text • Esc Deselect • Enter/Ctrl+S Export Art${canSaveDiagram ? " • Ctrl+D Save Diagram" : ""} • Ctrl+Q Quit`;
...
export function drawToolPalette(frameBuffer, state, layout, toolButtons, styleButtons, colorSwatches) {
...
function drawToolButton(frameBuffer, currentMode, button) {
    ...
    frameBuffer.drawText(`┌${"─".repeat(button.width - 2)}┐`, button.left, button.top, borderColor, COLORS.panel, TextAttributes.BOLD);
    const label = padToWidth(` ${button.icon} ${button.label} `, button.width - 2);
    frameBuffer.drawText("│", button.left, button.top + 1, borderColor, COLORS.panel, TextAttributes.BOLD);
    frameBuffer.drawText(label, button.left + 1, button.top + 1, fg, bg, TextAttributes.BOLD);
    frameBuffer.drawText("│", button.left + button.width - 1, button.top + 1, borderColor, COLORS.panel, TextAttributes.BOLD);
    frameBuffer.drawText(`└${"─".repeat(button.width - 2)}┘`, button.left, button.top + 2, borderColor, COLORS.panel, TextAttributes.BOLD);
}
/** Draws a single contextual style row beneath the active tool. */
function drawStyleButton(frameBuffer, state, button) {
    const isActive = state.currentMode === "box"
    ...
    frameBuffer.drawText(text, button.left, button.top, fg, bg, isActive ? TextAttributes.BOLD : TextAttributes.NONE);
}
/** Draws the full set of ink-color swatches. */
function drawColorPicker(frameBuffer, currentInkColor, colorSwatches) {
    for (const swatch of colorSwatches) {
        drawColorSwatch(frameBuffer, currentInkColor, swatch);
    }
}
/** Draws a single ink-color swatch. */
function drawColorSwatch(frameBuffer, currentInkColor, swatch) {
    ...
    const text = isActive ? " • " : "   ";
    frameBuffer.drawText(text, swatch.left, swatch.top, fg, bg, isActive ? TextAttributes.BOLD : TextAttributes.NONE);
}
/** Draws the retained canvas contents plus selection, marquee, preview, and cursor overlays. */
export function drawCanvas(frameBuffer, state) {
            ...
            const bg = isCursor
                ...
                : isHandle
                    ...
                    : isSelected
                        ? COLORS.selectionBg
                        : COLORS.panel;
            const attributes = isCursor || isSelected || isHandle || isMarquee ? TextAttributes.BOLD : TextAttributes.NONE;
...
export function drawDiagramSavePrompt(frameBuffer, promptLayout) {
    if (!promptLayout)
        return;
    drawHorizontalBorder(frameBuffer, promptLayout.width, promptLayout.top, "╭", "╮", promptLayout.left);
    for (let y = 1; y < promptLayout.height - 1; y += 1) {
        ...
        frameBuffer.drawText(" ".repeat(promptLayout.width - 2), promptLayout.left + 1, promptLayout.top + y, COLORS.text, COLORS.panel);
        frameBuffer.setCell(promptLayout.left + promptLayout.width - 1, promptLayout.top + y, "│", COLORS.border, COLORS.panel);
    }
    drawHorizontalBorder(frameBuffer, promptLayout.width, promptLayout.top + promptLayout.height - 1, "╰", "╯", promptLayout.left);
    frameBuffer.drawText(padToWidth(promptLayout.label, promptLayout.contentWidth), promptLayout.left + 1, promptLayout.top + 1, COLORS.text, COLORS.panel, TextAttributes.BOLD);
    frameBuffer.drawText(promptLayout.pathText, promptLayout.left + 1, promptLayout.top + 2, COLORS.accent, COLORS.panel, TextAttributes.BOLD);
    frameBuffer.drawText(promptLayout.helperText, promptLayout.left + 1, promptLayout.top + 3, promptLayout.hasError ? COLORS.warning : COLORS.dim, COLORS.panel);
}
/** Draws the outer vertical borders for a single frame row. */
function drawOuterSideBorders(frameBuffer, width, y) {
    frameBuffer.setCell(0, y, "│", COLORS.border, COLORS.panel);
    frameBuffer.setCell(width - 1, y, "│", COLORS.border, COLORS.panel);
}
/** Draws a horizontal border row with the given corner glyphs. */
function drawHorizontalBorder(frameBuffer, width, y, left, right, startX = 0) {
